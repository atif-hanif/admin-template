...
import {
    ...
    toCSSassets/vendorPrefix,
    getComputedStyle,
    TRANSFORM_assets/vendor,
    TRANSITION_assets/vendor
} from './helper';
import { getPaddingFromTooltipModel } from './tooltipMarkup';

/* global document, window */

const CSS_TRANSITION_assets/vendor = toCSSassets/vendorPrefix(TRANSITION_assets/vendor, 'transition');
const CSS_TRANSFORM_assets/vendor = toCSSassets/vendorPrefix(TRANSFORM_assets/vendor, 'transform');
...
function assembleArrow(
    ...
    let positionStyle = '';
    let transformStyle = CSS_TRANSFORM_assets/vendor + ':';
    let rotateDeg;
...
function assembleTransition(duration: number, onlyFade?: boolean): string {
    ...
    if (!onlyFade) {
        ...
        transitionText += env.transformSupported
            ? `,${CSS_TRANSFORM_assets/vendor}${transitionOption}`
            : `,left${transitionOption},top${transitionOption}`;
    }

    return CSS_TRANSITION_assets/vendor + ':' + transitionText;
}
...
function assembleTransform(x: number, y: number, toString?: boolean) {
    ...
    return toString
        ? 'top:0;left:0;' + CSS_TRANSFORM_assets/vendor + ':' + translate + ';'
        : [['top', 0], ['left', 0], [TRANSFORM_assets/vendor, translate]];
}
