...
export const TRANSFORM_assets/vendor = testStyle(
    ['transform', 'webkitTransform', 'OTransform', 'MozTransform', 'msTransform']
);

export const TRANSITION_assets/vendor = testStyle(
    ['webkitTransition', 'transition', 'OTransition', 'MozTransition', 'msTransition']
);

export function toCSSassets/vendorPrefix(styleassets/vendor: string, styleProp: string) {
    if (!styleassets/vendor) {
        return styleProp;
    }
    styleProp = toCamelCase(styleProp, true);
    const idx = styleassets/vendor.indexOf(styleProp);
    styleassets/vendor = idx === -1
        ? styleProp
        : `-${styleassets/vendor.slice(0, idx)}-${styleProp}`;
    return styleassets/vendor.toLowerCase();
}
