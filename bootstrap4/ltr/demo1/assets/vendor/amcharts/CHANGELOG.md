...
- Export: assets/images exported on zoomed-out pages sometimes were showing visual artifacts.
...
- It was not possible to add `Mapassets/images` to `Mapassets/imageseries` directly from the GeoJSON geodata.
...
- New `Popup` property: `dynamicResize`. If set to `true` (default) and contents contains unloaded assets/images it will resize itself when those finish loading.
...
### Changed
- `Popup` now will size itself to accommodate assets/images inside it as they are being loaded. To disable, set `Popup`'s `dynamicResize = false`. Also make sure you enable `maxWidth`/`maxHeight` in your CSS to avoid unreasonably large popups for unsized assets/images.
...
- Sometimes if using assets/imagesToConnect with MapLine of MapLineSeries, the lines were not drawn if the assets/imagesToConnect was set initially and not after the series initialized.
...
### Changed
- `ZoomControl`'s plus and minus button will not be exported to assets/images anymore.
...
- `MapLineSeries` with `assets/imagesToConnect` were not updating if assets/images changed their positions.
...
- `MapChart` sometimes could place Map assets/images at incorrect positions.
...
- Export: Exporting assets/images in IE9 was broken.
...
### Added
- New `Tooltip` proprety: `keepTargetHover` - will make target element hovered as long as its tooltip is hovered, good for bullets and map assets/images.
...
- Broken assets/images were causing export to fail.
...
### Changed
- [`MapLine.assets/imagesToConnect`](https://www.amcharts.com/docs/v4/reference/mapline/#assets/imagesToConnect_property) now accepts an array of string IDs that can reference assets/images by their `id` property.
...
- `useRetina` setting in `Export`. assets/images are now exported scaled up on retina displays by default. Set `useRetina = false` to disable.
...
- On retina displays assets/images will now export supersized in order not to lose quality. Set `useRetina = false` to disable.
...
- Changing data for `Mapassets/imageseries` and `MapLineSeries` used to result runtime error.
